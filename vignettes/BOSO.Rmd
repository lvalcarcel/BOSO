---
title: "Introduction to BOSO: Bilevel Optmization Selector of Operators"
author: "Luis V. Valcarcel & Xabier Cendoya & Francisco J. Planes"
date: "`r BiocStyle::doc_date()`"
abstract: >
  *BOSO* is an R package to perform a feature selection analysis in a linear regression problem.
  <br>
vignette: >
...
Note that it is necessary to have installed IBM ILOG CPLEX, with a version greater than 12.1 and also the *cplexAPI* package, which is used to invoke CPLEX from R environments.

# Introduction

The package is designed to operate one function to run all the algorithm at once. The main steps of the algorithm are the block strategy and the final problem.

* The block strategy is a pre-processing step in which the problem is divided in subproblems, to eliminate variables which are not selected by BOSO. Usual block size is 10 variables. This process is iterative, until convergence in number of selected variables.
* Final problem uses all the remaining variables
...
# Different parameters that can be changed in the algortihm.

Function `BOSO` has a lots of parameters that can be changed:

* *IC*  is information criterion to be used. Options are AIC, BIC and eBIC
* *IC.blocks*  is information criterion to be used in the block strategy step. By default, it is the same as the *IC*, but it can be changed. Additionally, if *IC* is eBIC, *IC.blocks* will be BIC.
* *nlambda* The number of lambda values, same as in `glmnet`.
* *nlambda.blocks* The number of lambda values, same as in `glmnet`. This parameter is by default 10, instead of 100, because here it is not necessary to obtain the best value for the betas, but to eliminate variables.
* *TH_IC* is the ratio over one that the information criterion must 
increase to be STOP. Default is 1e-3. Due to numerical tolerances, it is neccesary to set a threshold in order to detect the minimum value.

On the other hands, there are some parameters that are identical to the ones in the `glmnet` function: *lambda.min.ratio*,  *lambda*, *intercept*, *standardize* and *dfmax*.

Finally, some of the parameters that are solver specific:
...
# Example: high-5 scenario of main paper figures.

Given a synthetic data set (see Methods section in the paper), we can evaluate the performance as a feature selection method. First of all, we set the parameters for the generation of the synthetic data:

```{r define_data, eval=TRUE}

# Set some overall simulation parameters
n = 100; p = 10  # Size of training set, and number of variables
nval = n          # Size of validation set
nrep = 10          # Number of repetitions for a given setting
seed = 0          # Random number generator seed
s = 5             # Number of nonzero coefficients
beta.type = 1     # Simulation settings to consider
rho.vec = 0.35    # Pairwise variable correlation
snr.vec = exp(seq(log(0.05),log(6),length=10)) # Signal-to-noise ratios 
stem = paste0("sim.n",n,".p",p)

```

Now we are going to define the functions that are going to be evaluated. We are also inclusing Fordward Stepwise, Lasso and Relaxed lasso.
...
We have adapted the `sim.master`function from the *bestsubset* library function to evaluate the all the methods. `sim.master` is a function prepared to obtain all the scores from the different methods
...
sim.results <- list()
name = paste0(stem, ".beta", beta.type, sprintf(".rho%0.2f", rho.vec))

for (i in 1:length(snr.vec)) {
  cat(sprintf("SNR =  %f ( %i of %i) ...\n",round(snr.vec[[i]],2) ,i, length(snr.vec)))
  file = paste0("rds/", name, ".snr", round(snr.vec[[i]],2), ".rds")
  set.seed(i)
...
## Comparison of BOSO vs rest of algorithms

Finally, the next code chunk shows the comparison of BOSO(eBIC) with some of the most widely used algorithms in the literature: Lasso, Relaxed lasso and Forward Stepwise.
...
file.list <- file.path("rds", dir("rds"))

plot.BOSO <- list(
  "F" = plot.from.file(file.list = file.list, what="F", rel.to=NULL, tuning="val",
                      method.nums=method.nums, method.names=method.names),
  "nonzeros" = plot.from.file(file.list = file.list, what="nonzero", rel.to=NULL, tuning="val",
                             method.nums=method.nums, method.names=method.names),
  "fpos" = plot.from.file(file.list = file.list, what="fpos", rel.to=NULL, tuning="val",
                         method.nums=method.nums, method.names=method.names),
  "fneg" = plot.from.file(file.list = file.list, what="fneg", rel.to=NULL, tuning="val",
                         method.nums=method.nums, method.names=method.names))
...
The following chunk of code is designed to compare the performance of the BOSO algortihm using different information criteria. The options available are AIC, BIC and eBIC.
